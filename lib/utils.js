...
Utils.prototype.makeRequest = function(methodName, opt, retryCount, bc){
	...
	if('PartSize' in opt){
		stream = fs.createReadStream(opt.srcFile, {start:offset, end:offset+parseInt(opt.PartSize, 10)-1});
	}else if('ContentLength' in opt){
		stream = fs.createReadStream(opt.srcFile, {start:offset, end:offset+parseInt(opt.ContentLength, 10)-1});
	}
